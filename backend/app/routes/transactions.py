...
from app.schemas.transaction_schema import (
    ...
    TransactionFilterSchema, BulkTransactionSchema
)
from app.utils.decorators import validate_request, paginate, cache_control
...
def get_transactions():
    ...
    if sort == 'date':
        query = query.order_by(Transaction.date.desc() if order == 'desc' else Transaction.date)
    elif sort == 'amount':
        query = query.order_by(Transaction.amount.desc() if order == 'desc' else Transaction.amount)
    else:
...
def create_transaction():
    ...
    return jsonify(
        ...
        transaction=TransactionSchema().dump(tx)
    ), HTTP_STATUS.CREATED
...
def get_summary():
    ...
    return jsonify({
        ...
        'categories': [
            ...
            for k, v in sorted(cats.items(), key=lambda x: x[1]['amount'], reverse=True)
        ]
    }), HTTP_STATUS.OK
...
def get_recurring():
    ...
    return jsonify({
        'transactions': TransactionSchema(many=True).dump(tx)
    }), HTTP_STATUS.OK
...
def export_transactions():
    ...
    # Get transactions
    tx = Transaction.query.filter_by(user_id=user_id).order_by(Transaction.date.desc()).all()
