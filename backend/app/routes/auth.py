...
from app.utils.decorators import rate_limit, validate_request
...
def register():
    ...
    return (
        jsonify(message="User created", user=UserSchema().dump(user), tokens=token),
        HTTP_STATUS.CREATED,
    )
...
def login():
    ...
    return (
        jsonify(
            message="Login successful", user=UserSchema().dump(user), tokens=tokens
        ),
        HTTP_STATUS.OK,
    )
...
def update_current_user():
    ...
    return jsonify(message="Updated", user=UserSchema().dump(user)), HTTP_STATUS.OK
