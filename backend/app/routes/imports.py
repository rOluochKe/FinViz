...
def upload_file():
    ...
    return jsonify({"message": "File uploaded", "file": file_info}), HTTP_STATUS.OK
...
def get_template():
    ...
    if format == "csv":
        ...
        output.seek(0)
        return jsonify({"template": output.getvalue(), "format": "csv"})
    else:
        return jsonify({"template": template, "format": "json"})
