...
def system_stats():
    ...
    stats = {
        ...
        'transactions': db.session.execute('SELECT COUNT(*) FROM transactions').scalar(),
...
def cleanup():
    ...
    return jsonify({
        'message': f"Cleaned up {deleted} temp files"
    }), HTTP_STATUS.OK
...
def get_env():
    ...
    return jsonify({
        ...
        'database': str(current_app.config['SQLALCHEMY_DATABASE_URI']).split('@')[-1],
