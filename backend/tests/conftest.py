...
def test_categories(db_session, test_user):
    ...
    for cat_data in DEFAULT_CATEGORIES[:5]:
        category = Category(**cat_data, user_id=test_user.id, is_system=False)
...
def test_transactions(db_session, test_user, test_categories):
    ...
    for i in range(20):
        ...
        if i % 3 == 0:
            tx_type = "income"
            category = next(
                (c for c in test_categories if c.type.value == "income"),
                test_categories[0],
            )
            ...
            tx_type = "expense"
            category = next(
                (c for c in test_categories if c.type.value == "expense"),
                test_categories[0],
            )
